...
describe("fire embers", () => {
  ...
  it("kindles the floor voxel and recomputes the holder's block light", () => {
    ...
    expect(block.light.blocklight[block.light.paddedIndex(x, y, z)]).toBe(
    ...
    expect(block.light.blocklight[block.light.paddedIndex(x + 1, y, z)]).toBe(
      MAX_LIGHT - 1,
    );
  });
  ...
  it("puts the floor back when the fires go out", () => {
    ...
    expect(block.light.blocklight[block.light.paddedIndex(x, y, z)]).toBe(0);
  });
